...
public class GFrame {
	...
	public void onWidgetClick(GWidget widget) {
		...
		// If widget's starting position is the same as registered starting
		// position
	...
	public void showDiceResultsPopup(List<Integer> results) {
		String message = game.getActivePlayer().getName() + " threw "
				+ getIntegerListAsString(results);
		showMessagePopup(message, "OK", JOptionPane.INFORMATION_MESSAGE);
	}
	...
	private boolean showAcceptDoublingPopup() {
		String message = game.getInActivePlayer().getName()
				+ ", do you accept doubling the bet?";
		Object[] options = { "Yes", "No, I'll resign" };
		int answer = JOptionPane.showOptionDialog(frame, message,
				"Doubling the bet", JOptionPane.YES_NO_OPTION,
				JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
		if (answer == 0) {
			game.increaseBet();
			infoPanel.refresh();
	...
	private void showMessagePopup(String message, String buttonText,
		...
		JOptionPane.showOptionDialog(frame, message, "", JOptionPane.OK_OPTION,
